...
function Expand-CopawPackedEnvZip {
  param(
  ...
  $destFull = (Resolve-Path -LiteralPath $DestinationDir).Path
  $sizeMb = [math]::Round((Get-Item -LiteralPath $zipFull).Length / 1MB, 1)
  Write-Host "[build_win] Extracting ${sizeMb} MB zip -> $destFull (5–20+ min possible; no progress lines is normal)"
  $ok = $false
  if (Get-Command tar -ErrorAction SilentlyContinue) {
    Write-Host "[build_win] Using tar (faster than Expand-Archive for large conda-pack zips)..."
    & tar -xf $zipFull -C $destFull
    if ($LASTEXITCODE -eq 0) { $ok = $true }
    else { Write-Host "[build_win] tar failed (exit $LASTEXITCODE); falling back to Expand-Archive." -ForegroundColor Yellow }
  }
  if (-not $ok) {
    if (Test-Path $DestinationDir) { Remove-Item -Recurse -Force $DestinationDir }
    Expand-Archive -LiteralPath $zipFull -DestinationPath $destFull -Force
  }
  Write-Host "[build_win] Unpack complete."
}
...
$CondaUnpackAffectedPackages = @(
  ...
  "docker"           # constants.py: WINDOWS_LONGPATH_PREFIX (conda-pack #154)
  "pywin32"          # win32 path string literals
)
...
Expand-CopawPackedEnvZip -ZipPath $Archive -DestinationDir $Unpacked
...
if (Test-Path $CondaUnpack) {
  Write-Host "[build_win] Running conda-unpack (cwd=$EnvRoot; CPU may stay low, disk busy is normal)..."
  $cup = Start-Process -FilePath $CondaUnpack -WorkingDirectory $EnvRoot -NoNewWindow -PassThru
  while (-not $cup.HasExited) {
    Start-Sleep -Seconds 60
    Write-Host "[build_win] conda-unpack still running... $(Get-Date -Format 'HH:mm:ss')"
  }
  $cup.WaitForExit()
  ...
  if (Test-Path $WheelsCache) {
    ...
    foreach ($pkg in $CondaUnpackAffectedPackages) {
      ...
      if ($LASTEXITCODE -ne 0) {
        Write-Host "  Retrying $pkg from PyPI..." -ForegroundColor Yellow
    ...
    & $pythonExe -c "import docker.constants; print('✓ docker import OK')"
    if ($LASTEXITCODE -ne 0) {
      throw "CRITICAL: docker still has import errors after reinstall."
    }
    & $pythonExe -c "import win32api; print('✓ pywin32 import OK')"
    if ($LASTEXITCODE -ne 0) {
      throw "CRITICAL: pywin32 still has import errors after reinstall."
    }
