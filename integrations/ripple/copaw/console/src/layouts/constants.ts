...
export const UPDATE_MD: Record<string, string> = {
  zh: `### Ripple Console 如何更新

要更新 Ripple Console（运行时基于开源 CoPaw），可根据你的安装方式选择对应方法：
...
升级后重启服务（如 copaw app 或您的安装方式说明）。`,

  ru: `### Как обновить Ripple Console

Чтобы обновить Ripple Console (на базе открытого проекта CoPaw), выберите способ в зависимости от типа установки:
...
После обновления перезапустите сервис с помощью \`copaw app\`.`,
  ...
  en: `### How to update Ripple Console

To update Ripple Console (built on open-source CoPaw), use the method matching your installation type:
